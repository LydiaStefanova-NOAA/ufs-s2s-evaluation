...
# The scripts are prepared to handle variables on the list below 
 ...
 for domain in GlobalTropics ; do #Nino34 GlobalTropics Global ; do
 for season in AllAvailable; do
     bash pdf.sh whereexp=$whereexp whereobs=$whereobs varModel=tmpsfc reference=$reference domain=$domain hardcopy=$hardcopy season=$season nameModelA=$exp1 nameModelB=$exp2  ystart=$ystart yend=$yend mstart=$mstart mend=$mend mstep=$mstep dstart=$dstart dend=$dend dstep=$dstep mask=oceanonly
     bash pdf.sh whereexp=$whereexp whereobs=$whereobs varModel=prate reference=$reference domain=$domain hardcopy=$hardcopy season=$season nameModelA=$exp1 nameModelB=$exp2  ystart=$ystart yend=$yend mstart=$mstart mend=$mend mstep=$mstep dstart=$dstart dend=$dend dstep=$dstep mask=oceanonly
